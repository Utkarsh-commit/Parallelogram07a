...
// claimant (free guide form) right after they submit. Uses Brevo
// (https://www.brevo.com) transactional email API — 300 emails/day free,
// no domain verification required to start (but you DO need to verify
// your "from" sender address inside Brevo — see step 3 below).
//
// SETUP REQUIRED before this works:
// 1. Create a free account at https://www.brevo.com
// 2. Go to: Settings (gear icon, top right) → SMTP & API → API Keys →
//    Generate a new API key → copy it
// 3. Go to: Settings → Senders, Domains & Dedicated IPs → Senders → add
//    the email address you want to send FROM (e.g. your Gmail address,
//    parallelogram12107@gmail.com works fine for testing) → Brevo emails
//    that address a confirmation link → click it to verify.
// 4. In Netlify: Site settings → Environment variables → add:
//      Key:   BREVO_API_KEY
//      Value: <the key from step 2>
// 5. Update SEND_FROM_EMAIL below to the exact address you verified in
//    step 3.
// 6. Redeploy the site (Netlify → Deploys → Trigger deploy).
//
// This function only sends guides that actually exist as HTML files (see
// GUIDE_FILES below). Anything in the order that isn't in that list is
// silently skipped here — your existing WhatsApp notification / Formspree
// email still tells you about the full order either way, so you can
// deliver missing ones by hand.

const SEND_FROM_EMAIL = 'parallelogram12107@gmail.com'; // must be verified in Brevo (step 3 above)
...
exports.handler = async (event) => {
  ...
  console.log('[send-guide-email] BREVO_API_KEY present:', !!apiKey, apiKey ? '(len ' + apiKey.length + ')' : '');
  if (!apiKey) {
    console.log('[send-guide-email] ABORT: no BREVO_API_KEY in environment');
  ...
  try {
    attachments = await Promise.all(
      available.map(async (title) => {
        ...
        // Brevo wants attachments as { content: base64String, name: filename }
  ...
  try {
    console.log('[send-guide-email] calling Brevo API, to:', email);
    ...
    console.log('[send-guide-email] Brevo response status:', emailRes.status);

    if (!emailRes.ok) {
      const detail = await emailRes.text();
      console.log('[send-guide-email] Brevo ERROR body:', detail);
      return { statusCode: 502, headers, body: JSON.stringify({ error: 'Brevo request failed', detail }) };
    }

    const brevoBody = await emailRes.text();
    console.log('[send-guide-email] SUCCESS, Brevo response:', brevoBody);
    return { statusCode: 200, headers, body: JSON.stringify({ sent: true, delivered: available, skipped: unavailable }) };
  } catch (err) {
    console.log('[send-guide-email] ABORT: internal error calling Brevo:', String(err));
