...
// living at /guides/<file> on this same deployed site.
const GUIDE_FILES = {
...
exports.handler = async (event) => {
  ...
  if (available.length === 0) {
    // Nothing we can attach — seller's WhatsApp notification handles delivery manually.
    return { statusCode: 200, headers, body: JSON.stringify({ sent: false, reason: 'no_files_available' }) };
  }

  // Fetch each guide HTML file from the site's own /guides/ folder and base64-encode for attachment.
  let attachments;
  try {
    attachments = await Promise.all(
      available.map(async (title) => {
        const file = GUIDE_FILES[title];
        const res = await fetch(siteOrigin + '/guides/' + file);
